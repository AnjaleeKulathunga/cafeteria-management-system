...
import InitialNav from "../IntialNav/InitialNav";
import { useNavigate } from "react-router-dom";

function Home({ showNav = true }) {
    const { user , setUser } = useContext(StoreContext); // Get setToken from context
    ...
    <div className="home-container">      
      ...
      <div className="nav-options">
        ...
        <div className="nav-item">
          <h2>Complaint</h2>
          <p>Contact us for more details</p>
          <button onClick={()=> navigate("/complaintdetails")}>Complaint</button>
        </div>
        <div className="nav-divider"></div>
      <div className="nav-item">
      <h2>Feedback</h2>
      <p>Contact us for more details</p>
      <button onClick={() => navigate("/feedbackdetails")}>Feedback</button>
    </div>
